...
#define GPIO_PIN1 530  // Pin number on Raspberry Pi GPIO
#define GPIO_PIN2 531  // Pin number on Raspberry Pi GPIO
...
static ssize_t gpio_read(struct file *file, char __user *buf, size_t len, loff_t *offset) {
	int value = gpio_get_value(PIR_SENSOR);  // Read GPIO pin value
	char buffer[2];
...
static int __init gpio_driver_init(void) {
	...
	ret2 = gpio_request(GPIO_PIN2, "GPIO_DRIVER");
	ret3=gpio_request(PIR_SENSOR,"PIR_SENSOR");
	if (ret) {
	...
	ret2 = gpio_direction_output(GPIO_PIN2, 0);
	ret3=gpio_direction_input(PIR_SENSOR);
	if (ret) {
